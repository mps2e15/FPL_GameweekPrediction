...
def build_DNN(dense_dim=32, dropout=0.2,learning_rate=1e-2,seq_past_len=16,seq_fut_len=6):

    """Function to build simple DNN model"""
    ...
    #Flatten dynamic layers
    flattened_past = SequenceFlattener()([time_varying_known_past_inpt,time_varying_unknown_past_inpt])

    #Concatenate static with flat past data
    past_concat = tf.keras.layers.Concatenate(name='concat_past')([static_inpt,flattened_past])

    #Projected dense head
    out = TimeDistributedDense(n_future_steps=seq_fut_len, dense_dim=dense_dim, dropout=dropout)([past_concat,time_varying_known_future_inpt])

    #Compile the model
    model = tf.keras.Model([static_inpt,time_varying_known_past_inpt,time_varying_unknown_past_inpt,time_varying_known_future_inpt],\
                            out,
                            name='DNN')
...
def objective(trial):
    ...
    dense_dim = trial.suggest_categorical('dense_dim', [8, 16,32])
    dropout = trial.suggest_float('dropout', 0.05, 0.6, log=True)

    # Build model
    model = build_DNN(dense_dim=dense_dim, dropout=dropout,learning_rate=learning_rate,seq_past_len=16,seq_fut_len=6)
...
if __name__ == '__main__':
    ...
    #Fut the best model
    model = build_DNN(**study.best_trial.params)
    es = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10,min_delta=0.0025,restore_best_weights=True)
